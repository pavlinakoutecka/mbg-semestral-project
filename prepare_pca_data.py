...
def count_mutations_array(input_file, num_sequences, seq_length):
    ...
    seq_names = [0]*num_sequences
    sequences = [['-']*seq_length]*num_sequences  # cols = seq_length, rows = num_sequences
    ...
    with open(f"pca_input_data/{num_sequences}s_mutations_counts.csv", 'w', newline = '') as csvfile:
        ...
        my_writer.writerow(mutations_counts[1])
    csvfile.close()
...
def prepare_pca_data(input_file, output_file, num_sequences):
    ...
    with open(output_file, 'w', newline = '') as csvfile:
        ...
        for fasta in fasta_sequences:  # rows
            ...
            my_writer.writerow(seq_diffs)                   

    csvfile.close()

    print(f"PCA input data succesfully prepared and stored in file {output_file}.")
